...
export const authorizeFacilityType = (...facilityTypes) => {
  return (req, res, next) => {
    if (!req.user || !facilityTypes.includes(req.user.facilityType)) {
      return res.status(403).json({
        message: "Access denied. Facility role is not permitted for this resource.",
      });
    }
    next();
  };
